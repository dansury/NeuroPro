...
1. **Диаграмма и все показатели — математика, не нейросеть.** Геометрия диаграммы —
   `www/lib/chart.php`, а уровни выраженности, положение относительно медианы,
   достоверность, соотношение шкал между собой и разделы отчёта — `www/lib/metrics.php`.
...
www/                  # ← веб-корень; ровно это зеркалится на прод через pull.php
  lib/
    ...
    metrics.php       # ДЕТЕРМИНИРОВАННЫЙ расчёт всех показателей и их соотношений:
                      ...
                      #   достоверность, раздел отчёта, индексы. Нейросеть НЕ считает.
    ...
    prompts/          # Тексты автосидируемых версий промптов: *_v1.php (исходные,
                      ...
                      #   *_v3.php (активные: расчёт приходит готовым из metrics.php,
                      #   модель только пишет текст и не рисует таблиц)
