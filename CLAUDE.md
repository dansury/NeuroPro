...
www/                  # ← веб-корень; ровно это зеркалится на прод через pull.php
  lib/
    ...
    metrics.php       # ДЕТЕРМИНИРОВАННЫЙ расчёт всех показателей и их соотношений:
                      ...
                      #   оператора (Metrics::configure() из np_boot)
    ...
    matrix.php        # МАТЕМАТИЧЕСКАЯ матрица «когниция × эмоция» (SVG): X — ответы
                      ...
                      #   медианы, кружок разделён на секторы по СМК (Y оранжевый,
                      #   X синий, Z зелёный; фоновые — бледно-серые), размер — сумма
                      #   когнитивного и эмоционального ответа (контраст задаёт
                      #   мультипликатор), жирная тёмная обводка — p<0.05.
                      #   На странице результата пунктир ТАЩИТСЯ мышью: кружки
                      #   перекрашиваются на лету, значения сохраняются в settings.
                      #   В отчёте клиента матрица стоит ВМЕСТО таблиц
    phys.php          # Разбор OCR-текста таблицы значимости → «Знач.» + p + СМК (X/Y/Z) + «Балл» по осям
    ...
    prompts/          # Тексты автосидируемых версий промптов: *_v1.php (исходные,
                      ...
                      #   *_v5.php (активные: глубина отчёта у недорогих моделей —
                      #   скелет разбора шкалы, минимальный объём, СМК используется
                      #   всегда, рекомендации с конкретной практикой)
    interpret.php     # Сборка запроса + вызов нейросети + сохранение интерпретации
                      #   (в промпт уходят расчёт, итоги и описание матрицы — для
                      #   ЛЮБОГО теста одинаково)
    report.php        # Брендированный отчёт (Verdana) → HTML/PDF/письмо: диаграмма,
                      #   матрица, «Заключение»; таблиц нет. Плюс разбивка текста на
...
4. **Интерпретация.** По типу теста выбирается активная версия промпта;
   `Interpret::run()` отдаёт нейросети готовый расчёт (без сырых данных),
   результат сохраняется к версии.
