...
final class LLM {
     ...
     * @return array{rows: array<int, array{label:string,zna:?float,p:?string,sig:bool,smk:string}>, raw:string}
     */
...
4) Столбец «СМК» — порядок доминирования параметров, например «Z>X>Y», «X*>Z>Y», «X>YZ» («*» = значительно).
   СМК заполнен у КАЖДОЙ строки, в том числе там, где p>0.05 и строка напечатана бледно: читай его всегда, а не только у выделенных строк. Пустую строку «smk» возвращай, только если ячейка физически нечитаема.
5) Несколько числовых столбцов. Нужен РОВНО ОДИН из них — «Знач.» (он же «Значение»): число, ЗНАК и величина которого СОВПАДАЮТ с направлением и длиной графика.

ЕДИНСТВЕННЫЙ НУЖНЫЙ СТОЛБЕЦ — «Знач.». Все остальные числа скриншота ИГНОРИРУЙ полностью: столбцы «Балл», «m», номера строк, любые счётчики и суммы. Их не нужно возвращать, ими нельзя подменять «Знач.» и по ним нельзя ничего проверять. Балл ответа теста сервис берёт из выгрузки Excel, а не отсюда.

ГЛАВНОЕ ПРАВИЛО: знак «Знач.» бери ИЗ ГРАФИКА. Если столбик слева — «Знач.» отрицательное, даже если рядом стоит то же число без минуса. Если в строке несколько чисел, выбирай то, чей знак и величина согласуются с графиком; если ни одно не согласуется — верни null, а не соседний столбец.

Верни СТРОГО один JSON-объект без markdown:
{"rows":[{"label":"<точное название шкалы>","zna":<число со знаком или null>,"p":"<строка вида \\"<0.05\\"/\\">0.05\\"/\\"<0.01\\" или null>","smk":"<токен СМК или пустая строка>"}]}
Правила: числа — с точкой; если ячейка «Знач.» нечитаема — null; ничего не выдумывай. Порядок и названия строк — как в списке ожидаемых шкал ниже.
SYS;

        $user = [
            ['type' => 'text', 'text' => "Ожидаемые шкалы (верни строки в этом порядке и с этими названиями):\n" . $list
                . "\nПрочитай график каждой строки и верни JSON по схеме. Из чисел нужен только столбец «Знач.»."],
     ...
     * «Балл» не читается принципиально: со скриншота берётся только «Знач.»,
     * баллы теста приходят из выгрузки Excel — даже если модель вернула поле
     * «ball» по привычке, оно отбрасывается здесь.
     */
