...
final class Interpret {
    ...
    /**
     * Пометка модели у отчёта, который оператор получил в чужом чате и вставил
     * руками (окно вставки по кнопке «Скопировать промпт и данные»). Имя модели
     * сервису неизвестно, и подставлять модель из формы нельзя: это было бы
     * выдуманным данным (Конституция, принцип II).
     */
    public const MANUAL_MODEL = 'вставлено вручную';
    ...
    public static function buildStyleMessage(array $profile, ?array $phys, string $draft): string {
        ...
        // Последний раздел — единственное действие клиента. Редактор второго слоя
        // режет объём, и раздел из двух-трёх предложений первым просится под нож
        // или в общий список рекомендаций: называем его отдельным фактом.
        $fs = $m['first_step'] ?? null;
        $lines[] = '- ПОСЛЕДНИЙ РАЗДЕЛ «С чего начать» остаётся на месте и целиком: ровно одно действие с '
                 . 'выделенной жирным фразой-действием'
                 . ($fs ? ', опора — ' . $fs['label'] : '')
                 . '. Не сливать его с «Рекомендациями», не превращать в список и не сокращать до одной фразы.';
        // Второй слой — это и есть язык, поэтому указания по подаче нужны ему
        // не меньше, чем первому: иначе редактор ровняет любой отчёт под свой
        // средний стиль и сводит на нет то, что подобрал аватар клиента.
        $personaBlock = Persona::promptBlock(Persona::current((string) ($profile['name'] ?? '')));
        if ($personaBlock !== '') {
            $lines[] = '';
            $lines[] = $personaBlock;
        }
    ...
    public static function buildUserMessage(array $profile, ?array $phys = null): string {
        ...
        // Шкалы, которые в этой методике из отчёта выпадают целиком («не
        // упоминать в отчёте»), в промпт не отправляются вообще — не только
        // помечаются: лишний текст только повышал риск, что модель всё равно
        // упомянет тему, о которой её просили молчать. У методик со списком
        // непроявленных (Басса-Дарки) такие шкалы остаются — их печатают списком.
        $sendAxes = array_values(array_filter($m['axes'],
            static fn ($a) => $a['category'] !== 'skip' || $listsSkipped));

        $meanings = [];
        foreach ($sendAxes as $a) {
        ...
        foreach ($sendAxes as $a) {
            ...
            $extra = [];
            if ($a['category'] === 'skip') {
                // $listsSkipped === true здесь всегда (иначе шкала бы уже
                // отфильтровалась выше), но условие оставлено для ясности.
                $extra[] = 'РАЗДЕЛ: ' . $a['category_title'] . ' (только в общем списке, без разбора)';
            } else {
        ...
        // Шкалы, которым раздел не положен (категория «skip» у методик без
        // списка непроявленных), сюда уже не попадают — их вообще нет выше в
        // «РАСЧЁТЕ ПО ШКАЛАМ» (#1: подаём модели только то, что должно попасть
        // в отчёт, а не всё подряд с пометкой «не упоминай»).
        $lines[] = '';

        $lines[] = self::firstStepBlock($m);

        // КАК ПОДАВАТЬ МАТЕРИАЛ — цифровой аватар клиента (Persona). Считает его
        // код по всем тестам этого клиента, а не нейросеть по своему впечатлению
        // от текста: объём, язык, тон, опора и форма — такая же математика, как
        // уровни и разделы (Конституция, принцип I). Содержание отчёта аватар не
        // меняет: он про то, КАК написано, а не про то, ЧТО написано.
        $personaBlock = Persona::promptBlock(Persona::current((string) ($profile['name'] ?? '')));
        if ($personaBlock !== '') $lines[] = $personaBlock;
    ...
    /**
     * ОПОРА ПОСЛЕДНЕГО РАЗДЕЛА — шкала, вокруг которой строится единственное
     * действие «с чего начать». Выбирает её расчёт (Metrics::firstStep): какой
     * телесный сигнал самый достоверный и какая тема тяжелее всего — это
     * математика, а не литература (Конституция, принцип I). Модели остаётся
     * превратить готовый выбор в конкретный шаг.
     *
     * Само основание выбора («достовернее всех», «тяжелее всех») в отчёт не
     * идёт: клиенту нужен шаг, а не отчёт о том, как шаг выбирался.
     */
    private static function firstStepBlock(array $m): string {
        $fs = $m['first_step'] ?? null;
        $lines = [];
        $lines[] = 'С ЧЕГО НАЧАТЬ — ОПОРА ПОСЛЕДНЕГО РАЗДЕЛА (выбрана сервисом; другую шкалу не бери):';
        if (!$fs) {
            $lines[] = '- опорной шкалы нет: ни одна шкала не попала в разделы отчёта. Последний раздел всё равно '
                     . 'обязателен — дай одно спокойное общее действие, ничего не додумывая про отдельные шкалы.';
            $lines[] = '';
            return implode("\n", $lines);
        }
        $lines[] = '- шкала: ' . $fs['label']
                 . ($fs['category_title'] !== '' ? ' (раздел «' . $fs['category_title'] . '»)' : '') . ';';
        $lines[] = '- почему она: ' . $fs['basis_label'] . ';';
        $lines[] = '- её факт: ' . $fs['fact'] . ';';
        if (($fs['smk_label'] ?? '') !== '') {
            $lines[] = '- как устроена реакция: ' . $fs['smk_label']
                     . (($fs['smk_meaning'] ?? '') !== '' ? ' — ' . $fs['smk_meaning'] : '')
                     . ' (действие должно учитывать именно это устройство реакции);';
        }
        $lines[] = '- действие строится ВОКРУГ ЭТОЙ шкалы. Почему выбрана именно она, клиенту не объясняй: '
                 . 'ни «достоверно», ни «сильнее всего», ни «по данным физиологии» — просто предложи шаг.';
        $lines[] = '';
        return implode("\n", $lines);
    }
    ...
    private static function matrixBlock(array $m): string {
        ...
        $lines[] = '- ось ординат (вертикальная) — результаты теста, ответы (' . ($unit['title'] ?? '% от максимума шкалы') . ');'
        ...
        if (!empty($m['has_phys'])) {
            $lines[] = '- ось абсцисс (горизонтальная) — телесный отклик, «Знач.» на шкале ±' . Metrics::num((float) ($m['phys_scale'] ?? 0))
                     . '; медиана 0 — по центру, правее неё телесный отклик есть, левее — его нет;';
        } else {
            $lines[] = '- горизонтальной оси нет: физиология не распознана, показатели отложены только по вертикальной оси результатов теста;';
        }
    ...
    public static function mathLayer(array $m): array {
        return [
            ...
            // На какой шкале стоял первый шаг: пороги оператора могут сдвинуть
            // разделы, а вместе с ними и опору — снимок хранит ту, по которой
            // писался текст, который читал клиент.
            'first_step' => isset($m['first_step']) && is_array($m['first_step'])
                ? ['label' => $m['first_step']['label'], 'basis' => $m['first_step']['basis']]
                : null,
        ];
    }
}
