...
final class Interpret {
    ...
    private static function matrixBlock(array $m): string {
        ...
        $lines[] = '- ось X — когнитивный показатель, ответы теста (' . ($unit['title'] ?? '% от максимума шкалы') . ');'
                 . ' пунктир на ' . Metrics::num(Metrics::LOW_PCT) . ' % и ' . Metrics::num(Metrics::HIGH_PCT)
        ...
        $lines[] = '- цвет кружка — какой параметр преобладает: Y (эмоциональный отклик) красный, X (психическое '
