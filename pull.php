...
if ($config['secret'] !== '' && !hash_equals($config['secret'], (string)($_GET['token'] ?? ''))) {
    ...
    exit("<!doctype html><meta charset=\"utf-8\"><title>403</title><body style=\"background:#1e1e1e;color:#ff6b6b;font-family:monospace;padding:2rem\">forbidden</body>");
}
...
function handle_setup_post(string $configPath): void {
    ...
    if ($errors) {
        render_setup_form([
            ...
            'secret' => $secret, 'gh_token' => $ghToken,
...
function render_setup_form(array $values = [], array $errors = []): void {
    ...
    setup_field(
        'secret', 'URL secret',
        $secret, 'leave empty to allow anonymous pulls', true,
        [
            'Optional access gate. If set, every request must include <code>?token=&lt;this-value&gt;</code> — otherwise the script returns 403.',
            'Pick any random string. <strong>Strongly recommended in production</strong> so random visitors can\'t trigger pulls.',
            'Example: open a terminal and run <code>openssl rand -hex 16</code>, paste the output.',
        ]
    );
...
function render_setup_done(array $config): void {
    ...
    echo "</pre>";
    $url = 'pull.php' . ($config['secret'] !== '' ? '?token=' . urlencode($config['secret']) : '');
    echo "<a class=\"btn\" href=\"" . $h($url) . "\">$ run_pull_now</a>";
    echo "</div></body></html>";
}
